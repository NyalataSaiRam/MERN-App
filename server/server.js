...
import serviceAccountKey from "./blogging-website-d3255-firebase-adminsdk-xoaqm-b1d71354de.json" assert { type: "json" };
...
server.post("/signup", (req, res) => {
  ...
  if (fullname.length < 3) {
    return res
      .status(403)
      .json({ error: "Fullname must me atleast 3 letters long" });
  }
  if (!email.length) {
  ...
  User.findOne({ _id: req.user })
    .then((user) => {
      if (user.google_auth) {
        return res.status(403).json({
          error:
            "you can not change account's password because you logged in through google",
        });
      }
...
server.post("/update-profile", verifyJWT, (req, res) => {
  ...
  if (username.length < 3) {
    return res
      .status(403)
      .json({ error: "User name should be atleast 3 letters long" });
  }
  ...
  if (isAdmin) {
    ...
    if (!draft) {
      if (!des.length || des.length > 200) {
        return res.status(403).json({ error: "you must provide description" });
      }
      if (!banner.length) {
      ...
      if (!content.blocks.length) {
        return res.status(403).json({ error: "provide blog content" });
      }
      if (!tags.length || tags.length > 10) {
        return res.status(403).json({ error: "provide tags" });
      }
    }
    ...
    return res
      .status(500)
      .json({ error: "You don't have permissions to write blog" });
  }
});
...
server.post("/add-comment", verifyJWT, (req, res) => {
  ...
  if (!comment.length) {
    return res
      .status(403)
      .json({ error: "Write something inside comment box" });
  }
  ...
  new Comment(commentObj).save().then(async (commentFile) => {
    ...
    if (replying_to) {
      ...
      if (notification_id) {
        ...
        ).then((notification) => console.log("notification uploaded"));
      }
    }
...
const deleteComments = (_id) => {
  Comment.findOneAndDelete({ _id })
    .then((comment) => {
      ...
      Notification.findOneAndDelete({ comments: _id }).then((notification) =>
...
server.post("/delete-blog", verifyJWT, (req, res) => {
  ...
  if (isAdmin) {
    ...
    return res.status(500).json({error: "you don't have permission to delete blog"})
  }
});
