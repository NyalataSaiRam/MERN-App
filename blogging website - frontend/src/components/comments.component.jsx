...
const CommentContainer = () =>{
    ...
    const loadMoreComments = async ()=>{
        ...
        let newCommentArray = await fetchComments({skip:totalParentCommentsLoaded, blog_id:_id, setParentCommentCountFun:setTotalParentCommentsLoaded, comment_array: commentsArr})
    ...
    return(
        <div className={"max-sm:w-full fixed " + (commentsWrapper? "top-0 sm:right-0":"top-[100%] sm:right-[-100%]") + " duration-700 max-sm: right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
...

            {
                ...
                <button onClick={loadMoreComments} className="text-dark-grey p-2 px-3 hover:bg-grey/50 rounded-md flex items-center gap-2">
                    Load more
                </button>
                :""
