...
const CommentCard = ({index, leftVal, commentData}) => {
    ...
    const LoadMoreRepliesButton = () =>{
        ...
        let button = <button onClick={() => {loadReplies({skip:index-parentIndex, currentIndex:parentIndex})}} className="text-dark-grey py-2 px-3 hover: bg-grey/50 rounded-md flex items-center gap-2">Load More Replies</button>;
    ...
    return(
        <div className="w-full " style={{paddingLeft: `${leftVal*10}px`}}>
            ...
            <div className="my-5 p-6 rounded-md border border-grey">
                ...
                <p className="gelasio text-xl ml-3">{comment}</p>

                <div className="flex gap-5 items-center mt-5">

                    {
                        commentData.isReplyLoaded? 
                        <button onClick={hideReply} className="text-dark-grey p-2 px-3 bg-grey/50 rounded-md flex items-center gap-2">
                            <i className="fi fi-rs-comment-dots"></i>Hide Reply
                        </button>:
                        <button className="text-dark-grey p-2 px-3 bg-grey/50 rounded-md flex items-center gap-2" onClick={loadReplies} >
...

                    {
                        ...
                        <button onClick={deleteComments} className="p-2 px-3 rounded-md border border-grey ml-auto hover:bg-red/50 hover:text-red flex items-center">
